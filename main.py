...
logging.basicConfig(
    ...
    level=logging.INFO,  # Nivel de log, puedes usar DEBUG, INFO, WARNING, ERROR, CRITICAL
...
def verify_token(token: str = Header(...)):
    if token != API_TOKEN:
        logging.error("Intento de acceso con token inválido")  # Registrar un error
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token de autenticación inválido")
    logging.info("Token de autenticación válido")  # Registrar un acceso válido
    return token

@app.get("/")
def read_root():
    logging.info("Acceso a la ruta raíz")  # Registrar acceso a la ruta raíz
...
def generate(prompt: str = Form(...), max_length: int = Form(128), temperature: float = Form(0.5), top_p: float = Form(1.0), token: str = Depends(verify_token)):
    logging.info(f"Generando texto con el prompt: {prompt}")  # Registrar la entrada del prompt
    try:
        result = model_pipeline(prompt, max_length, temperature, top_p)
        logging.info("Texto generado correctamente")  # Registro si el modelo generó el texto correctamente
        return {"generated_text": result}
    except Exception as e:
        logging.error(f"Error al generar texto: {str(e)}")  # Registrar cualquier error
